...
import com.example.wetherapp.managers.DialogManager.Listener
...
class MainFragment : Fragment() {
    ...
    override fun onResume() {
        super.onResume()
        if (model.isUsingLocation.value == true) {  // Дописал: условие, чтобы не перезагружать GPS если выбран город вручную
    ...
    private fun updateCurrentCard() = with(binding){
        model.liveDataCurrent.observe(viewLifecycleOwner){
            ...
            // ДОБАВЛЕНО: Получаем SharedPreferences и ключи
            ...
            // ИЗМЕНЕНО: Форматируем значения перед отображением
            val maxTempDisp = UnitConverter.formatTemp(it.maxTemp, tempUnit)
            val minTempDisp = UnitConverter.formatTemp(it.minTemp, tempUnit)
            val currentTempDisp = UnitConverter.formatTemp(it.currentTemp, tempUnit)
            val feelsLikeDisp = UnitConverter.formatTemp(it.feelsLikeTemp, tempUnit)
            val windDisp = UnitConverter.formatWind(it.windKph, windUnit)
            val pressureDisp = UnitConverter.formatPressure(it.pressureMb, pressureUnit)
            val maxMinTemp = "$maxTempDisp / $minTempDisp"
            tvData.text = it.time
            tvCity.text = it.city
            tvCurrentTemp.text = currentTempDisp.ifEmpty { maxMinTemp }
            ...
            tvWind.text = if (it.windKph.isEmpty()) "" else windDisp + if (it.windDir.isNotEmpty()) " ${it.windDir}" else ""
            tvPressure.text = if (it.pressureMb.isEmpty()) "" else pressureDisp
            tvVisibility.text = if (it.visibilityKm.isEmpty()) "" else "${it.visibilityKm} km"
    ...
    private fun observePendingRequest() {
        model.pendingRequest.observe(viewLifecycleOwner) { request ->
            request?.let {
                ...
                model.clearPendingRequest()  // Дописал: метод в ViewModel для очистки
            }
        }
    }
    ...
    // Внутри класса MainFragment (после companion object или перед ним)

    private fun getConditionString(apiCondition: String): String {
        // 1. Создаем безопасный ключ ресурса:
        // "Partly cloudy" -> "condition_partly_cloudy"
        // "Patchy rain possible" -> "condition_patchy_rain_possible"
        ...
        // 2. Получаем ID ресурса по его имени
        ...
        // 3. Возвращаем переведенную строку или оригинал, если перевод не найден
        return try {
    ...
    private fun formatTime(apiTime: String, lang: String): String {
        ...
        return try {
            // 1. Формат, в котором приходит время от API
            ...
            // 2. Выбираем формат вывода на основе языка
            ...
            apiTime // В случае ошибки возвращаем как есть
        }
    }
    companion object {
        @JvmStatic
