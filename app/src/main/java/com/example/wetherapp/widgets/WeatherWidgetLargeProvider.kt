...
private const val ACTION_UPDATE_LARGE = "com.example.wetherapp.widget.UPDATE_LARGE"
...
class WeatherWidgetLargeProvider : AppWidgetProvider() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (intent.action == ACTION_UPDATE_LARGE) {
            val manager = AppWidgetManager.getInstance(context)
            val ids = manager.getAppWidgetIds(
                android.content.ComponentName(context, WeatherWidgetLargeProvider::class.java)
            )
    ...
    companion object {
        private fun updateWidget(context: Context, manager: AppWidgetManager, widgetId: Int) {
            ...
            val intent = Intent(context, WeatherWidgetLargeProvider::class.java).apply {
                action = ACTION_UPDATE_LARGE
            }
            ...
            val locationManager = LocationManager(
                ...
                onLocationReceived = { coords ->
                    requestWeather(context, coords) { city, temp, cond, icon, hum, wind ->
                        updateViews(context, manager, widgetId, city, temp, cond, icon, hum, wind)
                    }
                },
                onShowProgress = { },
                onShowGpsIndicator = { }
            ...
            val request = object : StringRequest(Request.Method.GET, url,
                { response ->
                    try {
                        ...
                        val hum = current.optString("humidity", "-")
                        val wind = current.optString("wind_kph", "-")
                        callback(city, "$tempC°C", cond, icon, "💧 $hum%", "🌬 $wind km/h")
                    } catch (e: Exception) {
                        Log.e("WidgetLarge", "Parse error", e)
                    }
                },
                { error ->
                    Log.e("WidgetLarge", "Network error: $error")
                }
            ) {
        ...
        private fun updateViews(
            ...
            views.setTextViewText(R.id.tvWidgetHumidity, hum)
            ...
            Thread {
                try {
                    ...
                    Log.e("WidgetLarge", "Image load failed", e)
                }
            }.start()
        }
    }
